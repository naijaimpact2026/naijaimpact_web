...
const ACTIVE_GROUPS = [
    { name: 'Philanthropists Circle', members: '2.4K', online: true, emoji: '💚' },
    { name: 'Entrepreneurs Network', members: '5.1K', online: true, emoji: '💼' },
    { name: 'Youth Impact Hub', members: '3.2K', online: true, emoji: '⭐' },
]
    ...
    return (
        <aside className="hidden xl:flex flex-col gap-4 w-72 shrink-0 pt-2">
            ...
            <div className="rounded-2xl bg-gradient-to-br from-primary/90 to-emerald-700 text-white p-4 shadow-sm relative overflow-hidden">
                ...
                <span className="absolute right-3 bottom-2 text-4xl opacity-20">🚀</span>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-border p-4 shadow-sm">
                ...
                <div className="flex items-center gap-1.5">
                    {ONLINE_USERS.map((u, i) => (
                        <div
                            key={i}
                            className={`relative w-9 h-9 rounded-full ${u.color} flex items-center justify-center text-white text-xs font-bold border-2 border-white`}
                        >
                            {u.initials}
                            <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-white" />
                        </div>
                    ))}
                    <div className="w-9 h-9 rounded-full bg-muted flex items-center justify-center text-xs font-semibold text-muted-foreground border-2 border-white">
                        +24
                    </div>
                </div>
            </div>
